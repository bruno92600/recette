    ...
    <div className={`flex flex-col rounded-md ${bg} overflow-hidden p-3 relative`}>
            ...
            className="relative h-32">
              ...
              <div className="absolute bottom-2 left-2 bg-white rounded-full p-1 cursor-pointer flex items-center gap-1 text-sm">
                <Soup size={16} /> {recipe.yield} portions
              </div>
              ...
              >
                {!isFavorite && <Heart size={20} className="hover:fill-red-600 hover:text-red-500" />}
                {isFavorite && <Heart size={20} className="fill-red-600 text-red-500" />}
              </div>
            </a>
            <div className="flex mt-1">
              <p className="font-bold tracking-wide">{recipe.label}</p>
            </div>
            <p className="my-2">Cuisine {recipe.cuisineType[0].charAt(0).toUpperCase() + recipe.cuisineType[0].slice(1)}</p>
